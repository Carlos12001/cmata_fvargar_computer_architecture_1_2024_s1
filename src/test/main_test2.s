...
reverberation:
  ...
  for_reverberation:
    ...
    if_offset_reverberation:
      cmp r8, r4        @ if offset>=k goto else_offset
      bgt else_load_data
      mov r3, #0         @ r3 is the y_old
      add r8, #4        @ increase offset+4 to reach k
      b end_if_offset_reverberation
    else_offset_reverberation:
      ldr r3, [r9]      @ search value y_old with n_k
    end_if_offset_reverberation:
    ...
    str r3, [r9]         @ save value y[n] on circular
    add r9, #4        @ increase n_k+4 to advanced in circular
    
    @ Check if n_k need to return original address circular
    if_nk_max_reverberation:
      add r0, r7, r12     @ max value address of circular (circular+circular_size)
      cmp r0, r9         @ if r0>=r9 goto end_if_nk_max
      bhs end_for_reverberation
      ldr r9, =circular   @ return to initial address circular
